...
<script type="text/javascript">
   $(function() {
    ...
    document.getElementById('sid').innerText=document.getElementById('asid').value;
...
<script type="text/javascript">
   $(function() {
    ...
    dataType:"text", //ajax返回值设置为text（json格式也可用它返回，可打印出结果，也可设置成json）
    success: function(json){ 
    	var addName = document.getElementById('selectName').value;
    	var addDiv='<div style="width:100px;float:left">'+addName+'</div>&nbsp';
...
<div class="container">
  ...
  <ul class="breadcrumb">
	...
	<div style="width:75%;border:1px solid #999999;padding:1% 1%;margin-top:-20px;margin-left:5%;float:left">
	<br/>
		<input type="hidden" id="courseId" value='<%=c.getId() %>'/>
		<input type="hidden" id="uname"/>
		<span class="infoTxt"><b>编号：</b></span>
		<span class="infoTxt"><%=c.getCid() %></span><br/><br/>
		<span class="infoTxt"><b>课程名：</b></span>
		<span class="infoTxt"><%=c.getCname() %></span><br/><br/>
		<span class="infoTxt"><b>课程教师：</b></span>
		<span class="infoTxt"><%=c.getTname() %></span><br/><br/>
		<span class="infoTxt" style="float:left;display:block"><b>课程助教：</b></span>
		<span class="infoTxt" style="float:left;display:block" id='assistant'><%=c.getAname() %></span>
		<div style="float:left;height:40px;margin-top:-10px;margin-left:2%">
			<form class="navbar-form navbar-left" role="search">
			<div class="form-group">
			  <input type="text" class="form-control" placeholder="Search By Id" id='asid'>
			</div>
			<input type="button" class="btn btn-default" data-target="#myModal3" data-toggle="modal" id="tj" value="查找"/>
			</form>
		</div>
		<% String info;
		if(c.getInfo()==null){info="暂无";}
		else{info = c.getInfo();}%>
		...
		<textarea name="word" style="width:550px;height:70px" id="courseInfo"><%=info %></textarea>
		</div>
		
		<div style="width:800px;float:left;margin-top:2%">
		<span class="infoTxt"  style="vertical-align:top;display:block;float:left"><b>参考书目：</b></span>	
			...
			<div id="collapseOne" class="panel-collapse collapse">
			  ...
			  <div id="numDiv">
			   <s:iterator value="#request.sidList" >
					<div style="width:100px;float:left"><s:property value="sid"/></div>&nbsp
					</s:iterator>
					</div>
					<div style="float:left;height:40px;margin-top:3%;">
						<form class="navbar-form navbar-left" role="search">
							<div class="form-group">
							  <input type="text" class="form-control" id="selectName" placeholder="请输入学号">
							</div>
						<input type="button" class="btn btn-default"  id="tj2" value="加入课程"/>
						</form>
					</div>
			  </div>
			</div>
		...
		<div style="width:800px;float:left;margin-top:1%;text-align:center">
		...
		<div class="modal-dialog" style="width:500px">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<span class="infoTxt" id='sid'></span><br/><br/>
					<span class="infoTxt">姓名：</span>
					<span class="infoTxt" id='username'></span><br/><br/>
					
					
				</div>
				<div class="modal-footer">
					<input type="button" class="btn btn-default btn-block" data-dismiss="modal" id='seleceAs' value='选择其为助教'/>
				</div>
			</div>
		</div>
		<!-- /.modal-content -->
